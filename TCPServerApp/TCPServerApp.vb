...
Public Class TCPServerApp
    ...
    Public Const DEFAULT_EXTERNAL_PORT As Long = 70000
    ...
    Public Class PayloadContainer
        ...
        Public Class PayloadCommandTypes
            ...
            Public Const BINARY_PATH = "BINARYPATH"
    ...
    Private Class ServerActions
        ...
        Private Sub Delete(key As String, ByRef thePayload As PayloadContainer)
            If Not theCtx._ObjInMemory.ContainsKey(key) Then
                'check the other external servers, to see if the data exists
                For Each serverAddress In theCtx._ExternalAddress
                    'Dim thePayload As New PayloadContainer
                    Dim serialPayload = serializer.Serialize(thePayload)

                    Dim theNewClient As New TCPClientApp.TCPClientApp({serialPayload & System.Environment.NewLine})

                    Try
                        Dim NewPayload As PayloadContainer = serializer.Deserialize(Of PayloadContainer)(theNewClient.ExecuteCommand(serverAddress.Key, DEFAULT_EXTERNAL_PORT))

                    Catch ex As Exception 'this will fail if the server doesn't own the key

                    End Try
                Next
                'if we get to this point, the current server, as well as extended servers 
                'don't have the requested key.  Throw the error
        ...
        Private Function Retrieve(key As String, ByRef thePayload As PayloadContainer) As PayloadContainer
            If Not theCtx._ObjInMemory.ContainsKey(key) Then
                'check the other external servers, to see if the data exists
                For Each serverAddress In theCtx._ExternalAddress
                    'Dim thePayload As New PayloadContainer
                    ...
                    Try
                        Dim NewPayload As PayloadContainer = serializer.Deserialize(Of PayloadContainer)(theNewClient.ExecuteCommand(serverAddress.Key, TCPServerApp.DEFAULT_PORT))
                        Return NewPayload
                    Catch ex As Exception 'this will fail if the server doesn't own the key
            ...
            Try
                Select Case thePayload.Command
                    ...
                    Case PayloadContainer.PayloadCommands.RETRIEVE
                        ...
                        Return Retrieve(key, thePayload)
        ...
        Private Function ExecuteCommand(ByRef payload As PayloadContainer) As Object
            ...
            Select Case payload.CommandType
                ...
                Case PayloadContainer.PayloadCommandTypes.BINARY_PATH
                    resultObj = ""
                    'resultObj = ExecuteFile(payload)
                Case Else
    ...
    Private Sub createTheServer(Optional ByVal theIPAddressString As List(Of String) = Nothing, _
        ...
        'Automatically add the internal port that allows communication between servers
        thePort.Add(DEFAULT_EXTERNAL_PORT)
        ...
        For Each address In theIPAddressString
            For Each port In thePort
                ...
                'don't add the port allowing communications between servers
                'to our list of available ports...
                'this list should only include ports for communication
                'with clients
                If port <> DEFAULT_EXTERNAL_PORT Then
                    _Port.Add(port)
                End If
